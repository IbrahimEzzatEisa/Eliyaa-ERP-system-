...
export class GregorianHijriCalendarComponent implements OnInit {
  ...
  selectedDayG;
  isHijri;
  @ViewChild('calenderInputH', {static: false}) calenderInputH: ElementRef;
  @ViewChild('calenderInputG' ,{static: false}) calenderInputG: ElementRef;
  @ViewChild('calenderH' , {static: false}) calenderH: ElementRef;
  @ViewChild('calenderG' , {static: false}) calenderG: ElementRef;
  @ViewChild('calenderIconG' , {static: false}) calenderIconG: ElementRef;
  @ViewChild('calenderIconH' , {static: false}) calenderIconH: ElementRef;
  @Input() gDate;
  @Input() disabled;
  ...
  getNextYear() {
    let month = this.currentPage.Gmonth;
    let year = this.currentPage.Gyear +1;
    this.makeDaysCalenderDateG(month, year);
  }
  getNextYearH() {
    let month = this.currentPageH.Hmonth;
    let year = this.currentPageH.Hyear +1;
    this.makeDaysCalenderDateH(month, year);
  }
  getPreviousYear() {
    let month = this.currentPage.Gmonth;
  ...
  dateToDBFormat(date) {
    ...
    if(!date) {
      ...
      dbFormat = {
        greg: this.formateDate(`${date.Gyear}/${date.Gmonth}/${date.Gday}`),
        hijri: this.formateDate(`${date.Hyear}/${date.Hmonth}/${date.Hday}`)
      } 
    }
    return dbFormat;
  }
